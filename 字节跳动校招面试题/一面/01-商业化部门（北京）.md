...
```
# 四、Vuex的使用和原理
![vuex](https://user-images.githubusercontent.com/72426886/132169045-836c27be-4f92-4a39-90bd-ec91b1f61025.png)
## 核心概念
...
## 原理
> Vuex只能使用在Vue上，因为其高度依赖于Vue的响应式系统和插件系统
### store的注入
> 调用applyMixin方法（现在的版本是Vuex.mix）在所有组件的beforeCreate生命周期注入this.$store这样一个对象
...
```
> 如何实现this.$store.state?


获取`this._VM._data.?state`
```js
...
Object.definProperties(Store.prototype,{
  state: {
    get: function(){
      return this._vm._data.$$state
    }
